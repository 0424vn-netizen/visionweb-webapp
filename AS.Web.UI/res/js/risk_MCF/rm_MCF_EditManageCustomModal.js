EditColumnModal = (function () {
    ...
    var RegisterRadTooltip = function (id, element) {
        var toolTip = $find(id);
        toolTip.hide();
        toolTip.set_targetControlID("");
    ...
    var uxGridListBox_OnClientLoad = function (sender, arg) {
        ...
        if (!!items && items.get_count() > 0) {
            ...
            items.forEach(function (item) {
                item.bindTemplate();
                setTooltipForGird(item.get_element().getAttribute("idtooltip"),
                item.get_element().getElementsByClassName("col-name")[0].getAttribute("id"));

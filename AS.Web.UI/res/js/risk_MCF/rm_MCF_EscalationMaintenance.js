...
function AddEscalation(btn) {
    var statusControl = document.getElementById(uxAddEscalationText_ClientID);
...
function DefaultEnterOnTextBoxEscalation(e) {
    ...
    isEnter = (keyCode == 13);
    if (isEnter) {
        $get(uxAddEscalation_ClientID).focus();
        $get(uxAddEscalation_ClientID).click();
        return false;
    }
}
...
function UpdateEscalation(statusControlID) {
...
function ActivateDeactivate(statusID, isActive) {
    if (isActive) // need to check before deactivating
        PageMethods.GetEscalationCountByEscalationStatus(statusID, isActive, GetEscalationCountByEscalationStatus_Callback);
    else
        UpdateIsActive(statusID, isActive);
}

function GetEscalationCountByEscalationStatus_Callback(result) {
    if (result[0] == 0) //no active escalation
        UpdateIsActive(result[1], result[2]);
    else
        ShowPopupModal("rm_MCF_Escalation_ActiveModal.aspx?" + result[1], 'auto');
}
...
function doValidInput(ele) {
    if ($.trim($(ele).parents('tr').find('.txtEditStatusName').val()).length == 0) {
        alert(rm_EscalationMaintenance_js_Escalation+' ' + Resources_ValMsg_Required);
        return false;
    }
    else if ($.trim($(ele).parents('tr').find('.txtEditStatusName').val()).indexOf('<') != -1 || $.trim($(ele).parents('tr').find('.txtEditStatusName').val()).indexOf('>') != -1) {
        alert(rm_EscalationMaintenance_js_Escalation+' ' + Resources_ValMsg_InvalidCharacter);
        return false;
    }
    else {
        return true;
...
function addCheckSpecialCharacters() {
    $('#' + uxAddEscalationText_ClientID).change(function (e) {
    ...
    $(".txtEditStatusName").each(function () {

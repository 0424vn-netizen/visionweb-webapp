...
$(document).ready(function () {
  ...
  datatarget.each(function () {
    ...
    $('.RadTabStrip li.rtsFirst').addClass('active'); //add class active for default tab strip
});
...
$(function () {
  ...
  $('#' + rm_RiskReport_uxFilteringOptionsContainer + ' input[type=text]').keypress(function (e) {
    ...
    $('#' + rm_RiskReport_uxMerchantNumber).change(function (e) {
        if (isIncludeSpecialCharacters(this)) {
            isCancelEnterRiskReport = true;
            CheckMerchantNumber(this.value);
            var newValue = removeSpecialCharacters(this);
            var uxMerchantNumber = $find(rm_RiskReport_uxMerchantNumber);
            uxMerchantNumber.set_value(newValue);
            setTimeout('isCancelEnterRiskReport = false;', 500);
        }
    });
});
...
function PopupModalClose(sender, eventArgs) {
  ...
  if (sender._navigateUrl.indexOf("DefaultSettingModal.aspx") >= 0) {
    ...
    if (sender._navigateUrl.indexOf("rm_MCF_ManageCustomViewsModal.aspx") >= 0) {
        if (parent.registerCloseCustomViewModalEvent)
            parent.registerCloseCustomViewModalEvent()
    }
    if (sender._navigateUrl.indexOf("rm_MCF_ManageTransactionHistoryCustomViewsModal.aspx") >= 0) {
        if (parent.registerCloseTHCustomViewModalEvent)
            parent.registerCloseTHCustomViewModalEvent()
    }
}
...
function getLinkBatchHistory(TerminalNumber, ReportDate, sender) {
  if (TerminalNumber == "" && ReportDate == "")
    return;
  $get(rm_RiskReport_hddValueLink).value = "B" + ";" + $(sender).text() + ";" + TerminalNumber + ";" + ReportDate;
  $get(rm_RiskReport_btnClickLink).click();
  return false;
}
function openTransactionHistoryModal(data) {
  if (data === "")
    return;
  $get(rm_RiskReport_hddValueLink).value = "T" + ";"+data;
  $get(rm_RiskReport_btnClickLink).click();
  return false;
}

function redirectTo() {
    window.location.href = rm_RiskReport_MerchantProfileURL;
}

$(window).load(function () {
    if ($(".as-animated-tabstrip").length > 0) {
...
let isBindSource = false;
function tabSelected(sender, e) {
    let tab = e.get_tab();
    $(".uc-content").addClass("hide");
    $("#" + tab.get_value()).removeClass("hide");
    $(".risk-report-tab .rtsLI").removeClass("active");
    let tabEl = tab.get_element();
    $(tabEl).addClass('active');

    if (tab.get_value() === "notescase" && !isBindSource) {
        isBindSource = true;
        setTimeout(function () {
            document.getElementById(uxFinishSaveDefaultSettingCH).click();
        }, 300)
        bindSourceAndRole(true);
    }
};
...
function ShowPopupAcctNumber(actt) {
    document.getElementById(rm_MCF_RiskReport_uxHiddenAccountNumberClick).value = actt;
    document.getElementById(rm_MCF_RiskReport_uxAccountNumberClick).click();
    return false;
}
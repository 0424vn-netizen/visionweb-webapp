function CheckPrefix() {
    ...
    if (!regName1.test($.trim($(MIF_MerchantDetails_FIS_uxtxtRelationShipManager).val().toLowerCase())) && !regName2.test($.trim($(MIF_MerchantDetails_FIS_uxtxtRelationShipManager).val().toLowerCase())) && !regName3.test($.trim($(MIF_MerchantDetails_FIS_uxtxtRelationShipManager).val().toLowerCase())))
        return true;
    else
        return false;
}
function CheckPrefixSpecial() {
    ...
    if (reg2.test($.trim($(MIF_MerchantDetails_FIS_uxtxtRelationShipManager).val())))
        return true;
    else
        return false;
}

...
function CompareToday(datePicker, type) {
    ...
    if (selectedDate > currentDate) {
        var object;
        switch (type) {
            case 0: object = ServiceMessage_Management_js_Daily_text; break;
            case 1: object = ServiceMessage_Management_js_Monthly_text; break;
            case 2: object = ServiceMessage_Management_js_Date_range_text; break;
        }
...
function CheckDate() {
    ...
    if (uxDaily && uxDaily.checked) {
        if (uxDate.get_textBox().value != "") {
            ...
            alert("Daily: " + Msg_ReportDate_InvaidDate);
            return false;
        }
    }
...
function CloseModalAndRebindGrid() {
    HidePopupModal();
    document.getElementById(uxRebind_ClientID).click();
}

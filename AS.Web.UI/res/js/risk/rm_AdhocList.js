...
if (!(typeof (UxExporter_OnResponseEnd) == 'undefined'))
...
function hyperLink_Click(merchantNumber) {
    var actionUrl = rootURL + "Risk/rm_AdhocList.aspx/MerchantNumberClick";
    $.ajax({
        type: "post",
        url: actionUrl,
        async: false,
...
function OpenRiskReport(merchantNumber, reportDate) {
    var exp = "rm_AdhocList.aspx";
    var expIndex = window.location.href.indexOf(exp);
    var http = window.location.href.substr(0, expIndex);
    window.location.href = http + "rm_RiskReport.aspx?MerchantNumber=" + merchantNumber;
    return false;
}

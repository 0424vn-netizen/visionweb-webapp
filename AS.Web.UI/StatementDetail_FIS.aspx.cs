...
public partial class StatementsDetailReport : NonReportPage
{
    ...
    private string MerchantNumber
    {
        get
        {
            return ViewState["vsMerchantNumber"] != null ? (string)ViewState["vsMerchantNumber"] : "";
        }
        set
        {
            ViewState["vsMerchantNumber"] = value;
        }
    }
    private DateTime dt
    {
        get
        {
            return ViewState["vsDate"] != null ? (DateTime)ViewState["vsDate"] : DateTime.MinValue;
        }
        set
        {
            ViewState["vsDate"] = value;
        }
    }
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            {
                MerchantNumber = SessionManager.CurrentReportFilter.Value;
                long temp;
    ...
    private void ExportToExcel()
    {
        ...
        string trueFileName = GetLocalResourceObject("StatementDetail_FIS_aspx_StatementFilename").ToString() + "_" + MerchantNumber + "_" + dt.Month.ToString("0#") + "_" + dt.Year.ToString();
        string filename = Server.MapPath(trueFileName);
        HttpContext.Current.Response.AppendHeader("content-disposition", VeraCodeSolution.RemoveCRLF(string.Format("attachment; filename=\"{0}.xls\"", trueFileName)));

...
public partial class StatementDetail_RS2 : ReportPage
{
    #region Enum
    ...
    protected void ButtonExcel_Click(object sender, EventArgs e)
    {
        ...
        DoDataBindControls(DataBindAction.IDINE, uxIDINE);
        ...
        DoDataBindControls(DataBindAction.CashAdvance, uxCashAdvance);
    ...
    protected void DoDataBindControls(Enum type, object sender)
    {
        ...
        switch ((DataBindAction)type)
        {
            ...
            case DataBindAction.IDINE:
                {
                    parameters.Add("@ReportType", 6, DbType.Int32);
                    DataSetIdine = WebServices.CsReportServices.GetReportsAsDataSet(SPA_STATEMENT_DETAIL, parameters);
                    if (DataSetIdine.Tables.Count > 0)
                    {
                        grid.DataSource = DataSetIdine.Tables[0];
                        if (DataSetIdine.Tables[0] != null && DataSetIdine.Tables[0].Rows.Count > 0)
                        {
                            grid.DataBind();
                        }
                    }
            ...
            case DataBindAction.CashAdvance:
                {
                    parameters.Add("@ReportType", 9, DbType.Int32);
                    DataSetCashAdvance = WebServices.CsReportServices.GetReportsAsDataSet(SPA_STATEMENT_DETAIL, parameters);
                    if (DataSetCashAdvance.Tables.Count > 0)
                    {
                        grid.DataSource = DataSetCashAdvance.Tables[0];
                        if (DataSetCashAdvance.Tables[0] != null && DataSetCashAdvance.Tables[0].Rows.Count > 0)
                        {
                            grid.DataBind();
                        }
                    }
    ...
    protected void uxDeposits_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        ...
        {
            if (DataSetDeposits != null && DataSetDeposits.Tables.Count > 1 && DataSetDeposits.Tables[1].Rows.Count > 0)
            {
    ...
    protected void uxAdjustments_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Footer)
        {
            if (DataSetAdjustments != null && DataSetAdjustments.Tables.Count > 1 && DataSetAdjustments.Tables[1].Rows.Count > 0)
            {
    ...
    protected void uxChargebacks_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Footer)
        {
            ...
            if (DataSetChargeback != null && DataSetChargeback.Tables.Count > 1 && DataSetChargeback.Tables[1].Rows.Count > 0)
            {
    ...
    protected void uxIDINE_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Footer)
        {
            if (DataSetIdine != null && DataSetIdine.Tables.Count > 1 && DataSetIdine.Tables[1].Rows.Count > 0)
            {
                DataTable data = DataSetIdine.Tables[1];
                ((Literal)e.Item.FindControl("uxLtrIDINE_SaleCount")).Text = VeraCodeSolution.ValidateResponseData(FormatData.FormatInteger(data.Rows[0]["SalesCount"]));
                ((Literal)e.Item.FindControl("uxLtrIDINE_SaleAmount")).Text = FormatData.FormatCurrency(data.Rows[0]["SalesAmount"], SessionManager.CurrencyFortmat);
                ((Literal)e.Item.FindControl("uxLtrIDINE_CreditAmount")).Text = FormatData.FormatCurrency(data.Rows[0]["CreditsAmount"], SessionManager.CurrencyFortmat);
                ((Literal)e.Item.FindControl("uxLtrIDINE_DiscountPD")).Text = FormatData.FormatCurrency(data.Rows[0]["DiscPaid"], SessionManager.CurrencyFortmat);
                ((Literal)e.Item.FindControl("uxLtrIDINE_NetDeposit")).Text = FormatData.FormatCurrency(data.Rows[0]["NetDeposit"], SessionManager.CurrencyFortmat);
            }
        }
    }

    protected void uxCashAdvance_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Footer)
        {
            if (DataSetCashAdvance != null && DataSetCashAdvance.Tables.Count > 1 && DataSetCashAdvance.Tables[1].Rows.Count > 0)
            {
                DataTable data = DataSetCashAdvance.Tables[1];
                ((Literal)e.Item.FindControl("uxCashAdvance_DepositAmount")).Text = VeraCodeSolution.ValidateResponseData(FormatData.FormatInteger(data.Rows[0]["DepositAmount"]));
                ((Literal)e.Item.FindControl("uxCashAdvance_AmountDeducted")).Text = VeraCodeSolution.ValidateResponseData(FormatData.FormatInteger(data.Rows[0]["AmountDeducted"]));
            }
        }
    }
    protected void uxReserveFund_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Footer)
        {
            if (DataSetReserveFund != null && DataSetReserveFund.Tables.Count > 1 && DataSetReserveFund.Tables[1].Rows.Count > 0)
            {
    ...
    protected void uxPlanSummary_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Footer)
        {
            if (DataSetPlanSummary != null && DataSetPlanSummary.Tables.Count > 1 && DataSetPlanSummary.Tables[1].Rows.Count > 0)
            {
    ...
    protected void uxAuthFees_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Footer)
        {
            if (DataSetFees != null && DataSetFees.Tables.Count > 0 && DataSetFees.Tables[0].Rows.Count > 0)
            {
    ...
    protected void uxInterchangeFees_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Footer)
        {
            if (DataSetFees != null && DataSetFees.Tables.Count > 1 && DataSetFees.Tables[1].Rows.Count > 0)
            {
    ...
    protected void uxTransactionFees_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Footer)
        {
            if (DataSetFees != null && DataSetFees.Tables.Count > 2 && DataSetFees.Tables[2].Rows.Count > 0)
            {
    ...
    protected void uxCardBrandFees_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Footer)
        {
            if (DataSetFees != null && DataSetFees.Tables.Count > 3 && DataSetFees.Tables[3].Rows.Count > 0)
            {
    ...
    private void BindData(bool isExportPdf)
    {
        ...
        DoDataBindControls(DataBindAction.AuthFees, isExportPdf ? uxAuthFees : uxAuthFees);
        DoDataBindControls(DataBindAction.InterFees, isExportPdf ? uxInterchangeFees : uxInterchangeFees);
        DoDataBindControls(DataBindAction.TransFees, isExportPdf ? uxTransactionFees : uxTransactionFees);
        DoDataBindControls(DataBindAction.CardBrandFees, isExportPdf ? uxCardBrandFees : uxCardBrandFees);
        DoDataBindControls(DataBindAction.OtherFees, isExportPdf ? uxOtherFees : uxOtherFees);

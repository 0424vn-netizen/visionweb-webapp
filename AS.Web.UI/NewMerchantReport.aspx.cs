...
[PagePermission("NewMerchRpt")]
public partial class NewMerchantReport : ReportPage
{
    #region Enum
    ...
    {
        REPORT_HEADER = GetLocalResourceObject("NewMerchantReport_aspx_cs_NewMerChantList").ToString() + " ";
            ...
            case DataBindAction.BindGrid:
                {
                    ...
                    string spName = "spa_cs_GetNewMerchantReport";
    ...
    private string GetActivity(bool isMerchant)
    {
        ...
        if (isMerchant)
        {
            value = GetLocalResourceObject("NewMerchantReport_aspx_cs").ToString() + " " + hierachyValue;
        }
        else
        {
            if (string.IsNullOrEmpty(hierachyValue))
            {
                if (GeneralFuncsLib.IsMerchantMode(hierachyMode))
                    value = GetLocalResourceObject("NewMerchantReport_aspx_cs_SearchAllMerchants").ToString();
                else
                    value = "Search all " + hierarchyModeValue + "s";
            }
            else
            {
                value = string.Format(GetLocalResourceObject("NewMerchantReport_aspx_cs_SearchBy").ToString(), hierarchyModeValue, hierachyValue);
            }
        }
        ...
        switch (ReportFilter.CurrentValue.DateOption)
        {
            case AS.Web.UI.Controls.DateOptionMode.Daily:
                {
                    beginDateText = ReportFilter.CurrentValue.DateOptionValue.From.ToGenericDateString();
                    dateText = string.Format(GetLocalResourceObject("NewMerchantReport_aspx_cs_On").ToString(), beginDateText);
                }
                break;
            case AS.Web.UI.Controls.DateOptionMode.Monthly:
                {
                ...
                dateText = string.Format(GetLocalResourceObject("NewMerchantReport_aspx_cs_FromTo").ToString(), beginDateText, endDateText);
                break;
            case AS.Web.UI.Controls.DateOptionMode.DateRange:
                {
                    beginDateText = ReportFilter.CurrentValue.DateOptionValue.From.ToGenericDateString();
        ...
        return string.Format(GetLocalResourceObject("NewMerchantReport_aspx_cs_NewMerchantReport").ToString(), value, dateText);

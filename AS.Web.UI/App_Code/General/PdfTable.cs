...
namespace WebSupergoo.ABCpdf9
{
	...
	public class PdfTable {
        ...
        /// </summary>
        public int HeaderLinePosition { get; set; }
        ...
        {
            get { return RowBottom.PosY - CellPadding; }
        }
		/// <summary>
		/// RowObject class describes row object (string or image)
		/// </summary>
		private class RowObject : IDisposable {
			public RowObject (){}
		...
		/// Header content
		/// </summary>
		private ArrayList mHeaderObjects = null;
        private string[] mHeaderBgColors = null;
        private string[] mHeaderColors = null;
        private bool mIsRootYSet = false;
        private double mRootY = 0;
		/// <summary>
        ...
        public PdfTable(Doc doc, int columns, double padding, double rootY)
        {
            mDoc = doc;
            if (rootY > 0)
            {
                XRect rect = new XRect(mDoc.Rect.String);
                mDoc.Rect.Inset(0, rect.Height - rootY + rect.Bottom);
                RowTop = new PagePos(this);
                mRowBottom = new PagePos(this);
                mDoc.Rect.SetRect(rect);
                SetRect(rect.String);    
            }
            else
            {
        ...
        public PdfTable(Doc doc, int columns, double padding)
            : this(doc, columns, padding, 0)
        {

        }
        public PdfTable(Doc doc, int columns)
            : this(doc, columns, 0, 0)
        {
        }
		/// <summary>
		...
		/// <returns>id of the created pdf object</returns>
 		public int AddText(string inText) {
			...
			if (theDrawn < inText.Length) {
				...
				using (XRect drawnRect = new XRect()) {
		...
		public void AddEnumerableData(IEnumerable data) {
			AddEnumerableData(data, true);
		}
 		
		private void AddEnumerableData(IEnumerable data, bool useHtml) {
		...
		/// </summary>
		/// <param name="data">Enumerable collection of strings and images</param>
		public void AddText(IEnumerable data) 
		{
			AddEnumerableData(data, false);
		}
        ...
        public void SetHeader(IEnumerable data, string[] bgColors, string[] colors)
        {
            mHeaderBgColors = bgColors;
            mHeaderColors = colors;
            ...
            if (mHeaderBgColors != null)
                for (int i = 0; i < mHeaderBgColors.Length; i++)
                {
                    if (!string.IsNullOrEmpty(mHeaderBgColors[i]))
                    {
                        FillCurrentColumn(mHeaderBgColors[i], i);
        ...
        {
            switch (HeaderLinePosition)
            {
                case 0:
                    FrameCells(inX1, inY1, inX2, inY2, true, true, true, true);
                    break;
                case 1:
                    FrameCells(inX1, inY1, inX2, inY2, true, false, false, false);
                    break;
                case 2:
                    FrameCells(inX1, inY1, inX2, inY2, false, true, false, false);
                    break;
                case 3:
                    FrameCells(inX1, inY1, inX2, inY2, false, false, false, false);
                    break;
            }
        }
        ...
        public void FrameColumns(int index)
        {
            FixRowPosition();
            for (int i = 0; i < mWidths.Length; i++)
                FrameCells(i, index, i, mRowPositions.Length - 1);
        }
		/// <summary>
		/// Frame all rows
		/// </summary>
        ...
        public void FillCurrentColumn(string inColor, int index)
        {
            FixRowPosition();
            FillCells(inColor, index, Row, index, Row);
        }
		/// <summary>

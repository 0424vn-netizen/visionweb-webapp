...
public partial class UserControls_MIF_MerchantDetails_MLS : ExportMultiSections
{
    #region Enums
    ...
    private const string FILE_NAME_BUSINESS_INFO = "~/App_Data/tpl_BusinessInformation_MLS.htm";
    private const string FILE_NAME_HIERARCHY_INFO = "~/App_Data/tpl_HierarchyInformation_MLS.htm";
    ...
    // SPA
    private const string SPA_GET_MERCHANT_PROFILE = "spa_cs_GetMerchantProfile_TSYS_Detail_MLS";
        ...
        switch ((DataBindAction)type)
        {
            case DataBindAction.BindMerchantDetails:
                ...
                if (dtMerch.Rows.Count > 0)
                {
                    ...
                    lnkLastBatch.Visible = !lnkLastBatch.Text.IsNullOrEmpty();
                    ...
                    {
                        MerchantProfileHelper.BuildMIFDetailHierarchyLink(dtMerch, item, "uxLinkBank", "EntityNumber1", HierarchyMode.BANK);
                        MerchantProfileHelper.BuildMIFDetailHierarchyLink(dtMerch, item, "uxLinkGroup", "EntityNumber2", HierarchyMode.GROUP);
                        MerchantProfileHelper.BuildMIFDetailHierarchyLink(dtMerch, item, "uxLinkAssociation", "EntityNumber3", HierarchyMode.ASSOCIATION);
                        MerchantProfileHelper.BuildMIFDetailHierarchyLink(dtMerch, item, "uxLinkChain", "Chain", HierarchyMode.CHAIN);
                        MerchantProfileHelper.BuildMIFDetailHierarchyLink(dtMerch, item, "uxLinkAccessChain", "EntityNumber4", HierarchyMode.ACCESSCHAIN);
                    }
                }
                else
                {
    ...
    protected string ButtonExcelHierarchyInformation_Click()
    {
        ...
        strExcelTemplate.Replace("[tpl_HierarchyInformation_htm_Bank]", Resources.Template.tpl_HierarchyInformation_FIS_htm_Bank);
        strExcelTemplate.Replace("[tpl_HierarchyInformation_htm_Group]", Resources.Template.tpl_HierarchyInformation_ORION_htm_Group);
        ...
        strExcelTemplate.Replace("[tpl_HierarchyInformation_htm_AccessChain]", this.GetLocalResourceObject("ltrSecondaryChain.Text").ToString());

        // Merchant Hierarchy
        strExcelTemplate.Replace("HI_BANK", "&nbsp;" + dataSource.Rows[0]["EntityNumber1"].ToString());
        strExcelTemplate.Replace("HI_GROUP", "&nbsp;" + dataSource.Rows[0]["EntityNumber2"].ToString());
        strExcelTemplate.Replace("HI_ASSOCIATION", "&nbsp;" + dataSource.Rows[0]["EntityNumber3"].ToString());
        strExcelTemplate.Replace("HI_CHAIN", "&nbsp;" + dataSource.Rows[0]["Chain"].ToString());
        strExcelTemplate.Replace("HI_ACCESSCHAIN", "&nbsp;" + dataSource.Rows[0]["EntityNumber4"].ToString());
    ...
    protected bool CheckPermissionAddEditAccessChain(object siteAccess)
    {
        return MerchantProfileHelper.CheckPermissionAddEditAccessChain(siteAccess, (SecurePage)Page);
    }
    protected void uxMerchantInfoItemDataBound(Object Sender, RepeaterItemEventArgs e)
    {
        ...
        //Secondary access chain
        PlaceHolder plAccessChain = ((PlaceHolder)e.Item.FindControl("uxPlaceAccessChain"));
        PlaceHolder plAccessChainNew = ((PlaceHolder)e.Item.FindControl("uxPlaceAccessChainNew"));
        Literal ltAccessChain = ((Literal)e.Item.FindControl("uxLHAccessChain"));
        // TK41447
            ...
            case ListItemType.AlternatingItem:
                {
                    ...
                    //Secondary chain
                    if (row["EntityNumber4"].ToString().IsNullOrEmpty()
                        && Page.IsUserWithPermission("AddEditAccessChain")
                        && MerchantProfileHelper.SiteAccessIsOptInOut(row["SiteAccess"]))
                    {
                        plAccessChain.Visible = false;
                        plAccessChainNew.Visible = true;
                        ltAccessChain.Visible = false;
                    }
                    else
                    {
                        plAccessChainNew.Visible = false;
                        if (CheckHierarchy("Chain"))
                        {
                            plAccessChain.Visible = true;
                            ltAccessChain.Visible = false;
                        }
                        else
                        {
                            plAccessChain.Visible = false;
                            ltAccessChain.Visible = true;
                        }
                    }
                }
                break;

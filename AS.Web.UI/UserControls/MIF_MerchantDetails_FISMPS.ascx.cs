...
public partial class UserControls_MIF_MerchantDetails_FISMPS : ExportMultiSections
{
    #region Constants

    private string PATH_EXPORT_EXCEL_FILE = ConfigurationManager.AppSettings["ExportTempFolder"];
    private const string FILE_NAME_BI = "~/App_Data/tpl_BusinessInformation_FISMPS.htm";
    private const string FILE_NAME_HIERARCHY_INFO = "~/App_Data/tpl_HierarchyInformation_FISMPS.htm";
    private const string FILE_NAME_BANK_INFO = "~/App_Data/tpl_BankInformation_FISMPS.htm";
    private const string FILE_NAME_OTHER_CARD_INFO = "~/App_Data/tpl_OtherCardInformation_FISMPS.htm";
    ...
    private const string MPS_CHAIN = "MPSChain";
    private const string AGENT = "Agent";

    private const string OPTED_IN = "Opted In";
    private const string BUSINESS_INFO = "Business Information";
    private const string HIERARCHY_INFO = "Hierarchy Information";
    #endregion Constants
    ...
    protected override void OnDataBindControls(Enum type, object sender)
    {
        ...
        switch ((DataBindAction)type)
        {
            case DataBindAction.BindMerchantDetails:
                ...
                string spa_name = "spa_cs_GetMerchantProfile_FISMPS";
                ...
                if (dtMerch.Rows.Count > 0)
                {
                    ...
                    lnkLastBatch.Visible = !BindValue("LastBatchActivity").Trim().IsNullOrEmpty();
                    if (lnkLastBatch.Visible)
                    {
                        lnkLastBatch.Text = FormatDate(BindValue("LastBatchActivity").Trim());
                    }
                            ...
                            && !MerchantNumber.Equals(ReportPage.ReportFilter.CurrentValue.Value)))
                    {
                        ...
                        DateTime.TryParse(FormatDate(dtMerch.Rows[0]["LastBatchActivity"]), out tempDate);
    ...
    protected string ButtonExcelHierarchyInformation_Click()
    {
        ...
        strExcelTemplate.Replace("[tpl_HierarchyInformation_FIS_htm_Agent]", Resources.Template.tpl_HierarchyInformation_FIS_htm_Agent);
        strExcelTemplate.Replace("[tpl_HierarchyInformation_FIS_htm_MICChain]", Resources.Template.tpl_HierarchyInformation_FIS_htm_MICChain);
        ...
        strExcelTemplate.Replace("HI_MIC_CHAIN_CODE", "&nbsp;" + _datasource.Rows[0][MPS_CHAIN].ToString());
    ...
    protected string ButtonExcelMerchantInformation_Click()
    {
        ...
        if (uxpnlRMCtrls.Visible)
        {
            ...
            strExcelTemplate.Replace("HI_RELATIONSHIP_MANAGER", "&nbsp;" + uxlbRelationshipManager.Text);
        }
        else
        {
            strExcelTemplate = new StringBuilder(
        ...
        strExcelTemplate.Replace("HI_LAST_BATCH_ACTIVITY", "&nbsp;" + FormatDate(_datasource.Rows[0]["LastBatchActivity"]));

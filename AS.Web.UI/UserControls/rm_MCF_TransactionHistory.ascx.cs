...
public partial class UserControls_rm_MCF_TransactionHistory : GlobalUserControl
{
    ...
    const string IMAGE = "<a class=\"image-link\" href=\"#\"  onclick=\" return ShowPopupModalChild({0},'{1}','auto');\"><img src='../res/img/information.png' border=\"0\"/></a>&nbsp; &nbsp;";
    int index = 0;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (FlagLink == 1)
        {
            ...
            string url = string.Format("<span class=\"dark-blue\"><a class=\"link-back\" href=\"#\" onclick=\"OpenInstanceWindow('rm_MCF_TransactionHistoryModal.aspx?{0}','DQMCFWindow'); return false;\">[", queryString);
    ...
    protected void uxExport_OnNeedExportConfig(object sender, ExportConfig exportConfig)
    {
        ...
        exportConfig.FileName = GeneralFuncsLib.FormatFileName(exportConfig.FileName);
    ...
    protected void uxTransactionDetails_NeedDataSource(object source, GridNeedDataSourceEventArgs e)
    {
        ...
        uxTransactionDetails.DataSourceInvoker = new ASFuncInvoker(WebServices.RiskServices,
                    ...
                    new object[] { "spa_GetTransactionHistory", 
            ...
            case GridItemType.Item:
                {
                    ...
                    dupe.ToolTip = VeraCodeSolution.DoVeraCode(string.Format("{0:C}", rowItem["DupeAmount"]));
                    ...
                    {
                        //dataItem["ADF"].Attributes.Add("style", "background-color: Red !important;");
                        //dataItem[REASON_COLUMN].BackColor = Color.Red;
                        dataItem["ADF"].Text = string.Format("<span style='border-bottom: 2px red solid !important;'>{0}</span>", rowItem["ADF"]);
                    }
                    if (GeneralFuncsLib.NvlString(rowItem[TRANS_TYPE]).Equals("Sales"))
                    {
                        //dataItem[TRANS_TYPE].BackColor = Color.Green;
                        //dataItem[TRANS_TYPE].Attributes.Add("style", "background-color: Green !important;");
                        dataItem[TRANS_TYPE].Text = string.Format("<span style='border-bottom: 2px green solid !important;'>{0}</span>", rowItem[TRANS_TYPE]);
                        
                    }
                    else if (GeneralFuncsLib.NvlString(rowItem[TRANS_TYPE]).Equals("Auth"))
                    {
                        //dataItem[TRANS_TYPE].BackColor = Color.LightBlue;
                        //dataItem[TRANS_TYPE].Attributes.Add("style", "background-color: LightBlue !important;");
                        dataItem[TRANS_TYPE].Text = string.Format("<span style='border-bottom: 2px lightblue solid !important;'>{0}</span>", rowItem[TRANS_TYPE]);
                    }
                        ...
                        else
                        {
                            if (GeneralFuncsLib.HasIPForFullCard((SecurePage)this.Page))
                            {
                                dataItem["PartialCardNumber"].Text = VeraCodeSolution.DoVeraCode(
                                    String.Format(IMAGE, (index + 1), BuildUrlForFullCard(rowItem["ReportType"].ToString(), rowItem["RecordId"].ToString(), rowItem["IssuingBank"].ToString(), rowItem["ReportDate"].ToString())) + VeraCodeSolution.DoVeraCode(urlCard + rowItem["PartialCardNumber"].ToString() + "</a>")
                                    );
                            }
                            else
                            {
    ...
    {
        string path = "~/App_Data/TransactionHistoryColumn.xml";
        ...
        if (nodeList != null)
        {
            foreach (XmlNode node in nodeList)
            {
                if (!string.IsNullOrEmpty(node.Attributes["Text"].Value))
                    node.Attributes["Text"].Value = GetLocalResourceObject(node.Attributes["Text"].Value).ToString();// GetGlobalResourceObject("Template", node.Attributes["Text"].Value).ToString();
            }
        }
        string strXml = doc.OuterXml;
        uxOrderBy1.LoadXml(strXml);
        uxOrderBy2.LoadXml(strXml);
        uxOrderBy3.LoadXml(strXml);

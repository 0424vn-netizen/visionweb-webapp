...
public partial class UserControls_MIF_MerchantDetails_PLANET : ExportMultiSections
{
    #region Constants

    // Export file name
    private string PATH_EXPORT_EXCEL_FILE = ConfigurationManager.AppSettings["ExportTempFolder"];
    private const string FILE_NAME_BUSINESS_INFO = "~/App_Data/tpl_BusinessInformation_PLANET.htm";
    private const string FILE_NAME_HIERARCHY_INFO = "~/App_Data/tpl_HierarchyInfomation_PLANET.htm";

    private const string FILE_NAME_MERCHANT_INFO = "~/App_Data/tpl_MerchantInformation_PLANET.htm";
    private const string FILE_NAME_MERCHANT_INFO_REP = "~/App_Data/tpl_MerchantInformation_PLANET_Rep.htm";

    //private const string OPTED_IN = "Opted In";

    //SPA
    private const string SPA_GET_MERCHANT_PROFILE = "spa_cs_GetMerchantProfile_PLANET";
    ...
    {
        return MerchantProfileHelper.GetRoutingNumberFromFullCard(full);
    }
    ...
    protected override Dictionary<int, Func<string>> GetListFunctions()
    {
        ...
        if (IsRiskInformation)
        {
            ...
            exportFucntions.Add(4, ButtonExcelMerchantMemos_Click);
        }
        else
        {
            exportFucntions.Add(2, ButtonExcelHierarchyInformation_Click);
            exportFucntions.Add(3, ButtonExcelMerchantMemos_Click);
        }
    ...
    public void DoMultiExportExcel()
    {
        ...
        exportNames.Add(0, GetLocalResourceObject("LiteralResource24.Text").ToString());
        exportNames.Add(1, GetLocalResourceObject("MIF_MerchantDetails_PLANETCS_Text_BusinessInformation").ToString());

        if (IsRiskInformation)
        {
            exportNames.Add(2, GetLocalResourceObject("MIF_MerchantDetails_Text_RiskInformation").ToString());
            exportNames.Add(3, GetLocalResourceObject("MIF_MerchantDetails_PLANETCS_Text_HierarchyInformation").ToString());
            exportNames.Add(4, GetLocalResourceObject("MIF_MerchantDetails_PLANETCS_Text_Memos").ToString());
        }
        else
        {
            exportNames.Add(2, GetLocalResourceObject("MIF_MerchantDetails_PLANETCS_Text_HierarchyInformation").ToString());
            exportNames.Add(3, GetLocalResourceObject("MIF_MerchantDetails_PLANETCS_Text_Memos").ToString());
        }
    ...
    {
        dt = MerchantProfileHelper.ProcessNullValue(dt);
        return (dt == null || dt.ToString().IsNullOrEmpty())
            ? string.Empty : Convert.ToDateTime(dt).ToString(WebSiteConstants.DATE_FORMAT);
    }
    ...
    protected string ButtonExcelBusinessInformation_Click()
    {
        ...
        strExcelTemplate.Replace("[tpl_BusinessInformation_PLANET_htm_MerchantName]", Resources.Template.tpl_BusinessInformation_PLANET_htm_MerchantName);
        ...
        strExcelTemplate.Replace("BI_MERCHANT_NAME", "&nbsp;" + dataSource.Rows[0]["MerchantName"].ToString());
        ...
        {
            strExcelTemplate.Replace("BI_SITE_ACCESS", "&nbsp;" + GetLocalResourceObject("MIF_MerchantDetails_PLANETJS_Text_NotAvailable").ToString());
        }

        strExcelTemplate.Replace("BI_SIC_MCC",
            "&nbsp;" + FormatSIC(dataSource.Rows[0]["SICMCC"].ToString(),
            dataSource.Rows[0]["SICDescription"].ToString()));
        string excelFileName = GeneralFuncsLib.GetFileName(GetLocalResourceObject("MIF_MerchantDetails_PLANETCS_Text_BusinessInformation").ToString());
    ...
    protected string ButtonExcelHierarchyInformation_Click()
    {
        ...
        strExcelTemplate.Replace("[tpl_HierarchyInfomation_PLANET_htm_SalesUnit]", Resources.Template.tpl_HierarchyInfomation_PLANET_htm_SalesUnit);
        strExcelTemplate.Replace("[tpl_HierarchyInfomation_PLANET_htm_Agent]", Resources.Template.tpl_HierarchyInfomation_PLANET_htm_Agent);
        ...
        strExcelTemplate.Replace("HI_SALES_UNIT", "&nbsp;" + _datasource.Rows[0]["HierarchyID1"].ToString());
        strExcelTemplate.Replace("HI_AGENT", "&nbsp;" + _datasource.Rows[0]["HierarchyID2"].ToString());
        string excelFileName = GeneralFuncsLib.GetFileName(GetLocalResourceObject("MIF_MerchantDetails_PLANETCS_Text_HierarchyInformation").ToString());
        return WriteContentToFile(PATH_EXPORT_EXCEL_FILE, excelFileName, "xls", strExcelTemplate.ToString());
    }
    ...
    protected string ButtonExcelMerchantInformation_Click()
    {
        ...
        string excelFileName = GeneralFuncsLib.GetFileName(GetLocalResourceObject("LiteralResource24.Text").ToString());
        return WriteContentToFile(PATH_EXPORT_EXCEL_FILE, excelFileName, "xls", strExcelTemplate.ToString());
    }

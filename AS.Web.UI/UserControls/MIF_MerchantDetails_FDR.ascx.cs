...
public partial class UserControls_MIF_MerchantDetails : ExportMultiSections
{
    ...
    private const string FILE_NAME_MERCHANT_CARD_INFO =
        "~/App_Data/tpl_MerchantCardInformation.htm";
    ...
    protected override void OnDataBindControls(Enum type, object sender)
    {
        ...
        switch ((DataBindAction)type)
        {
            case DataBindAction.BindMerchantDetails:
                ...
                string spaName = IsCaseManagement ? "spa_cm_GetMerchantProfileFDRForCaseManagement" : "spa_cs_GetMerchantProfile";
                ...
                if (dtMerch.Rows.Count > 0)
                {
                    ...
                    if (!IsCaseManagement)
                    {
                        uxCustomReport.Visible = (dtMerch.Rows[0]["MerchantType"].ToString() == "H"
                        ...
                        // Visibility of Report Access based on uxCustomReport panel
                        ShowHideReportAccess((dtMerch.Rows[0]["MerchantType"].ToString() == "H"
                            && (SessionManager.CurrentUserType == WebSiteEnums.UserHierarchyMode.AS
                                || SessionManager.CurrentUserType == WebSiteEnums.UserHierarchyMode.CS)));
                    }
                    ...
                    foreach (RepeaterItem item in rptMerchantInfo.Items)
                    {
                        ...
                        if (aSalesAgent != null && CheckHierarchy(HierarchyMode.SALES_AGENT))
                        {
                            if (!string.IsNullOrEmpty(dtMerch.Rows[0][SALES_AGENT].ToString()))
                            {
                                aSalesAgent.Attributes.Add(
                                    "onclick",
                                    MerchantProfileHelper.BuildSubmitReportFilterJavascriptCall(
                                        HierarchyMode.SALES_AGENT,
                                        dtMerch.Rows[0][SALES_AGENT].ToString())
                                );
                            }
                            else
                            {
                                PlaceHolder pSalelAgent = ((PlaceHolder)item.FindControl("uxHSalesAgent"));
                                pSalelAgent.Visible = false;
                            }
                        }
                        if (aAgent != null && CheckHierarchy(HierarchyMode.AGENT))
                        {
                            if (!string.IsNullOrEmpty(dtMerch.Rows[0][SYS_PRIN_AGENT].ToString()))
                            {
                                aAgent.Attributes.Add(
                                    "onclick",
                                    MerchantProfileHelper.BuildSubmitReportFilterJavascriptCall(
                                        HierarchyMode.AGENT,
                            ...
                            {
                                PlaceHolder plAgent = ((PlaceHolder)item.FindControl("uxHAgent"));
                                plAgent.Visible = false;
                            }
                        }
                        if (aHead != null && CheckHierarchy(HierarchyMode.HEADQUARTER))
                        {
                            if (!string.IsNullOrEmpty(dtMerch.Rows[0][HEAD_QUARTER].ToString()))
                            {
                                aHead.Attributes.Add(
                                    "onclick",
                                    MerchantProfileHelper.BuildSubmitReportFilterJavascriptCall(
                                        HierarchyMode.HEADQUARTER,
                                        dtMerch.Rows[0][HEAD_QUARTER].ToString()));
                            }
                            else
                            {
                                PlaceHolder plHeadquater = ((PlaceHolder)item.FindControl("uxHHeadquaster"));
                                plHeadquater.Visible = false;
                            }
                        }
                    }
                }
                ...
                {
                    //bind card info                
                    rptCardInfo.DataSource = GetMerchantCardInfo();
                    rptCardInfo.DataBind();

                    //bind pricing info
                    rptPricingInfo.DataSource = GetPricingInformation();
                    rptPricingInfo.DataBind();

                    //bind pin debit info
                    rptPinDebit.DataSource = GetMerchantCardInfo_PinDebit();
                    rptPinDebit.DataBind();
                }
                else
                {
                    rptCardInfo.Visible = false;
                    rptPricingInfo.Visible = false;
                    rptPinDebit.Visible = false;
                }
    ...
    private DataTable GetMerchantCardInfo()
    {
        return WebServices.CsReportServices.GetReports("spa_cs_GetMerchantProfile_MerchantCardInfo", GetCardInfoParams());
    }
    private DataTable GetPricingInformation()
    {
        return WebServices.CsReportServices.GetReports("spa_cs_GetMerchantProfile_PricingInformation", GetCardInfoParams());
    }
    private DataTable GetMerchantCardInfo_PinDebit()
    {
        return WebServices.CsReportServices.GetReports("spa_cs_GetMerchantProfile_MerchantCardInfo_PinDebit", GetCardInfoParams());
    }
    private FilterParameterCollection GetCardInfoParams()
    {
        FilterParameterCollection parameters = new FilterParameterCollection();
        parameters.AddLoggedInUserReportingParams();
        parameters.AddLoggedInUserPrimaryUserID();
    ...
    protected override void OnPostBackActions(Enum type, object sender)
    {
        switch ((PostBackAction)type)
        {
            case PostBackAction.UpdateEmail:
                FilterParameterCollection parameters = new FilterParameterCollection();
                FilterParameterCollection parameters_out = new FilterParameterCollection();
                parameters.AddLoggedInUserReportingParams();
                parameters.Add(new FilterParameter("@MerchantNumber", ReportPage.ReportFilter.CurrentValue.Value, DbType.AnsiString));
                parameters.Add(new FilterParameter("@RecipientEmail", RecipientEmail, DbType.String));
                parameters.Add(new FilterParameter("@CustomReport", CustomReport, DbType.Boolean));
                WebServices.MsReportServices.ExecuteNonQueryCommand(SPA_UPDATE_CUSTOM_CHAIN_REPORT, parameters, out parameters_out);

                int PermissionId = 0;
                parameters.Clear();
                parameters_out.Clear();
                parameters.Add(new FilterParameter("@PermissionCode", "ManCusChain", DbType.AnsiString));
                DataTable table = WebServices.CsReportServices.GetReports("spa_GetPermissionID", parameters);
                if (table != null && table.Rows.Count > 0)
                    Int32.TryParse(table.Rows[0]["PermissionId"].ToString(), out PermissionId);

                if (chkCustomReport.Checked)
                {
                    WebServices.SecurityServices.DeleteExcludePermissionForUser(
                ...
                {
                    WebServices.SecurityServices.InsertExcludePermissionForUser(
                        SessionManager.CurrentUser.ASClient,
                        ReportPage.ReportFilter.CurrentValue.Value,
                        PermissionId,
                        DateTime.Now,
                ...
                //Secondary
                UserCollection secondUsers = WebServices.SecurityServices.GetSecondaryUsers(
                    SessionManager.CurrentUser.ASClient,
                    ReportPage.ReportFilter.CurrentValue.Value);
                if (secondUsers != null && secondUsers.Count > 0)
                {
                    foreach (User u in secondUsers)
                    {
                        if (chkCustomReport.Checked)
                        {
                            WebServices.SecurityServices.DeleteExcludePermissionForUser(
                                   SessionManager.CurrentUser.ASClient, u.UserID,
                                   PermissionId, SessionManager.CurrentUser.RecId, false);
                        }
                        else
                        {
                            WebServices.SecurityServices.InsertExcludePermissionForUser(
                                   SessionManager.CurrentUser.ASClient, u.UserID,
                                   PermissionId, DateTime.Now, DateTime.Now,
                                   SessionManager.CurrentUser.RecId, false);
                        }
                    }
                }
                uxEmailRecipents.Enabled = chkCustomReport.Checked;
                break;
        }
    }

    protected void uxUpdateEmail_Click(object sender, EventArgs e)
    {
        OnPostBackActions(PostBackAction.UpdateEmail);
    }
    ...
    public void Rebind()
    {
        ...
        {
            uxPlhSwitchTSYSMID.Visible = true;
            this.BindSwitchMID();
        }
        else
        {
            uxPlhSwitchTSYSMID.Visible = false;
        }
    }
    ...
    public void DoMultiExportExcel()
    {
        ...
        exportNames.Add(1, GetLocalResourceObject("MIF_MerchantDetails_FDRCS_Text_BusinessInformation").ToString());
        if (IsRiskInformation)
        {
            exportNames.Add(2, GetLocalResourceObject("MIF_MerchantDetails_Text_RiskInformation").ToString());
            exportNames.Add(3, GetLocalResourceObject("MIF_MerchantDetails_FDRCS_Text_HierarchyInformation").ToString());
            exportNames.Add(4, GetLocalResourceObject("MIF_MerchantDetails_FDRCS_Text_BankInformation").ToString());
            exportNames.Add(5, GetLocalResourceObject("MIF_MerchantDetails_FDRCS_Text_OtherCardInformation").ToString());
            exportNames.Add(6, GetLocalResourceObject("MIF_MerchantDetails_FDRCS_Text_PricingInformation").ToString());
            ...
            exportNames.Add(9, GetLocalResourceObject("MIF_MerchantDetails_FDRCS_Text_Memos").ToString());
        }
        else
        {
            exportNames.Add(2, GetLocalResourceObject("MIF_MerchantDetails_FDRCS_Text_HierarchyInformation").ToString());
            exportNames.Add(3, GetLocalResourceObject("MIF_MerchantDetails_FDRCS_Text_BankInformation").ToString());
            exportNames.Add(4, GetLocalResourceObject("MIF_MerchantDetails_FDRCS_Text_OtherCardInformation").ToString());
            exportNames.Add(5, GetLocalResourceObject("MIF_MerchantDetails_FDRCS_Text_PricingInformation").ToString());
            exportNames.Add(6, GetLocalResourceObject("MIF_MerchantDetails_FDRCS_Text_MerchantCardInformation").ToString());
            exportNames.Add(7, GetLocalResourceObject("MIF_MerchantDetails_FDRCS_Text_MerchantCardInformation").ToString() + " - " +
               GetLocalResourceObject("MIF_MerchantDetails_FDRCS_Text_PINDebit").ToString());
            exportNames.Add(8, GetLocalResourceObject("MIF_MerchantDetails_FDRCS_Text_Memos").ToString());
        }
    ...
    protected string ButtonExcelBusinessInformation_Click()
    {
        ...
        {
            strExcelTemplate.Replace("BI_SITE_ACCESS", "&nbsp;" + GetLocalResourceObject("MIF_MerchantDetails_FDRCS_Text_NotAvailable").ToString());
        }
        ...
        string excelFileName = GeneralFuncsLib.GetFileName(GetLocalResourceObject("MIF_MerchantDetails_FDRCS_Text_BusinessInformation").ToString());
    ...
    protected string ButtonExcelHierarchyInformation_Click()
    {
        ...
        strExcelTemplate.Replace("HI_SYS_PRIN_AGENT", "&nbsp;" + _datasource.Rows[0]["SysPrinAgent"].ToString());
        strExcelTemplate.Replace("HI_HEADQUARTER_MERCHANT", "&nbsp;" + _datasource.Rows[0]["Headquarter"].ToString());
        strExcelTemplate.Replace("HI_MERCHANT_TYPE", _datasource.Rows[0]["MerchantType"].ToString());
        strExcelTemplate.Replace("HI_CHAIN_CODE", "&nbsp;" + _datasource.Rows[0]["ChainCode"].ToString());
    ...
    protected string ButtonExcelBankInformation_Click()
    {
        ...
        string excelFileName = GeneralFuncsLib.GetFileName(GetLocalResourceObject("MIF_MerchantDetails_FDRCS_Text_BankInformation").ToString());
        return WriteContentToFile(PATH_EXPORT_EXCEL_FILE, excelFileName, "xls", strExcelTemplate.ToString());
    }
    ...
    {
        DataTable _datasource = GetPricingInformation();
        if (_datasource == null || _datasource.Rows.Count == 0)
            return string.Empty;
        StringBuilder strExcelTemplate = new StringBuilder(
    ...
    {
        DataTable _datasource = GetMerchantCardInfo();
        if (_datasource == null || _datasource.Rows.Count == 0)
            return string.Empty;
        StringBuilder strExcelTemplate = new StringBuilder(
    ...
    {
        DataTable _datasource = GetMerchantCardInfo_PinDebit();
        if (_datasource == null || _datasource.Rows.Count == 0)
            return string.Empty;
        StringBuilder strExcelTemplate = new StringBuilder(
    ...
    protected string ButtonExcelMerchantInformation_Click()
    {
        ...
        {
            strExcelTemplate = new StringBuilder(File.ReadAllText(Server.MapPath(FILE_NAME_MERCHANT_INFO_REP)));
        ...
        {
            strExcelTemplate = new StringBuilder(File.ReadAllText(Server.MapPath(FILE_NAME_MERCHANT_INFO)));
        }
    ...
    private void BindSwitchMID()
    {
        FilterParameterCollection parameters = new FilterParameterCollection();
        parameters.AddLoggedInUserReportingParams();
        parameters.Add(new FilterParameter("@MerchantNumber", ReportPage.ReportFilter.CurrentValue.Value, DbType.AnsiString));
        parameters.Add(new FilterParameter("@Processor", BackEndProcessor.FDR, DbType.String));
        ...
        {
            uxPlhSwitchTSYSMID.Visible = true;
            uxSwitchTSYSMID.NavigateUrl = "#";
            uxSwitchTSYSMID.Attributes.Add("onclick", MerchantProfileHelper.BuildSubmitReportFilterJavascriptCall(HierarchyMode.MERCHANT_NR, data.Rows[0][TSYS_MID].ToString()));
        }
        else
        {
            uxPlhSwitchTSYSMID.Visible = false;

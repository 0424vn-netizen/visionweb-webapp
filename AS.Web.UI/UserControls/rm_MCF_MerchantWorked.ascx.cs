...
public partial class UserControls_rm_MCF_MerchantWorked : GlobalUserControl
{
    #region Constants
        ...
        switch ((MerchantWorkedType)Type)
        {
            case MerchantWorkedType.MerchantWorkedPage:
                if (e.Item is GridDataItem)
                {
                    ...
                    string urlCard = string.Format("<a href=\"javascript:void(0)\" onclick=\"openPopupWindow('rm_MCF_RiskReport.aspx?{0}','RiskReport'); return false;\">", queryString);
            ...
            case MerchantWorkedType.MerchantWorkedModal:
                if (e.Item is GridDataItem)
                {
                    GridDataItem dataItem = e.Item as GridDataItem;
                    DataRowView dataRow = e.Item.DataItem as DataRowView;
                    bool isLink = dataRow["IsEnabledURL"].ToString() == "1";
                    if (isLink)
                    {
                        ...
                        string urlCard = string.Format("<a  href=\"javascript:void(0)\" onclick=\"CloseModal(this);parent.selectMerchantWorked('rm_MCF_DQNextQReportPopup.aspx?{0}'); return false;\" >", queryString);
                        dataItem["MerchantNumber"].Text = VeraCodeSolution.DoVeraCode(string.Format("{0}{1}</a>", urlCard, dataRow["MerchantNumber"].ToString()));
                    }
                    else
                        dataItem["MerchantNumber"].Text = dataRow["MerchantNumber"].ToString();
                }
                break;
        }
    ...
    {
        string spaName = "spa_RM_MCF_Get_MerchantWorkedReport";
    ...
    {
        string spaName = "spa_RM_MCF_Mgmt_GetUserListWorkedReport";
        FilterParameterCollection parameters = new FilterParameterCollection();

...
public partial class UserControls_rm_MCF_MerchantClassification_Attribute : GlobalUserControl
{
    ...
    private const string CT_METRIC_TO = "uxMetricTo";
    private const string CT_METRIC_FROM = "uxMetricFrom";
    private const string CT_METRIC_TO_LABEL = "uxLabelMetricTo";
    private const string CT_METRIC_FROM_LABEL = "uxLabelMetricFrom";
    ...
    protected void uxAttributeList_SelectedIndexChanged(object sender, RadComboBoxSelectedIndexChangedEventArgs e)
    {
        ...
        if (combo.SelectedItem != null)
        {
            ...
            switch (fieldType)
            {
                ...
                case AttributeRiskScoreMode.SingleSelect:
                case AttributeRiskScoreMode.OperandIfEmpty:
                    ...
                    break;
                case AttributeRiskScoreMode.OperandMetric:
                    GetOperandMetric(combo.SelectedValue);
                    LoadOperand(combo.SelectedValue, item);
                    break;
            }
    ...
    protected void operand_SelectedIndexChanged(object sender, RadComboBoxSelectedIndexChangedEventArgs e)
    {
        ...
        AttributeRiskScoreMode fieldType = (AttributeRiskScoreMode)int.Parse(attrs.SelectedItem != null ? attrs.SelectedItem.Attributes["FieldType"].ToString() : "0");
        if (fieldType == AttributeRiskScoreMode.OperandMetric)
        {
            ShowHideOperandMetric(combo, item);
        }
        else
        {
            if (fieldType == AttributeRiskScoreMode.SingleSelect)
                LoadMetric(attrs.SelectedValue, combo.SelectedValue, "SingleSelect", item, fieldType);
            else
                LoadMetric(attrs.SelectedValue, combo.SelectedValue, combo.SelectedItem.Text, item, fieldType);
            TextBox metric_textList = item.FindControl(CT_METRIC_TEXTLIST) as TextBox;
            HiddenField metric_textListHide = item.FindControl(CT_METRIC_TEXTLISTHIDE) as HiddenField;
            if (metric_textList != null)
            {
    ...
    protected void uxAttributeList_ItemDataBound(object sender, RadComboBoxItemEventArgs e)
    {
        ...
        e.Item.Attributes["MinValue"] = dataItem["MinValue"] != null ? dataItem["MinValue"].ToString() : string.Empty;
        e.Item.Attributes["MaxValue"] = dataItem["MaxValue"] != null ? dataItem["MaxValue"].ToString() : string.Empty;
    }
    ...
    private void LoadMetric(string attrId, string operandId, string text, GridDataItem item, AttributeRiskScoreMode fieldType)
    {
        ...
        {
            if (config.Metrics != null || fieldType == AttributeRiskScoreMode.OperandIfEmpty)
            {
                ...
                {
                    list = config.Metrics.GetMetricByOperand(operandId);
                }
                if ("=".Equals(text) && !text.Trim().ToLower().Equals("SingleSelect"))
    ...
    private void GetOperandMetric(string attributeId)
    {
        ...
        if (ViewState[classAttrId] == null)
        {
            ...
            DataTable tb = WebServices.RiskServices.GetReports("spa_RM_MCF_MRS_Get_MetricOperandByAttribute", parameters);
    ...
    private void StoreGridValues()
    {
        foreach (GridDataItem item in uxAttributeDataGrid.Items)
        {
            ...
            if (row != null)
            {
                ...
                if (fieldType == AttributeRiskScoreMode.OperandMetric)
                {
                    TextBox mectric_from = item.FindControl(CT_METRIC_FROM) as TextBox;
                    TextBox mectric_to = item.FindControl(CT_METRIC_TO) as TextBox;
                    ...
                    if (oper.SelectedItem.Text.Trim().Contains("<>") && !string.IsNullOrEmpty(mectric_from.Text))
                    {
                        mectricfrom = int.Parse(mectric_from.Text);
                        mectricto = int.Parse(mectric_to.Text);
                        mectric = string.Format("{0};{1}", mectricfrom, mectricto);
                    }
                    else if (!string.IsNullOrEmpty(mectric_from.Text))
                    {
                        mectricfrom = int.Parse(mectric_from.Text);
                        mectric = mectric_from.Text;
                    }
                    row[PAGE_GRID_OBJECT.METRICVALUE] = mectric;
                    row[PAGE_GRID_OBJECT.FROM_VALUE] = mectricfrom;
                    row[PAGE_GRID_OBJECT.TO_VALUE] = mectricto;
                    row[PAGE_GRID_OBJECT.OPERAND] = oper.SelectedValue;
                }
                else if (fieldType == AttributeRiskScoreMode.Operand || fieldType == AttributeRiskScoreMode.SingleSelect || fieldType == AttributeRiskScoreMode.OperandIfEmpty)
                {
                    RadComboBox oper = item.FindControl(CT_OPENRAND) as RadComboBox;
                    RadComboBox metric = item.FindControl(CT_METRIC) as RadComboBox;
    ...
    private void SetData(GridDataItem item)
    {
        ...
        if (fieldType == AttributeRiskScoreMode.OperandMetric)
        {
            ...
            ShowHideOperandMetric(oper, item);
            mectric_from.Text = metrics[0];
            if (metrics.Length > 1)
            {
                mectric_to.Text = metrics[1];
            }
        }
        else if (fieldType == AttributeRiskScoreMode.Operand || fieldType == AttributeRiskScoreMode.SingleSelect || fieldType == AttributeRiskScoreMode.OperandIfEmpty)
        {
            ...
            LoadMetric(attr.SelectedValue, dataRow[PAGE_GRID_OBJECT.OPERAND].ToString(),
                fieldType == AttributeRiskScoreMode.SingleSelect ?
                "SingleSelect" : oper.SelectedItem.Text, item, fieldType);
            ...
            if (!"=".Equals(oper.SelectedItem.Text) || fieldType == AttributeRiskScoreMode.SingleSelect)
            {
        ...
        foreach (GridDataItem item in uxAttributeDataGrid.Items)
        {
            ...
            if (attr.SelectedValue != "-1")
            {
                ...
                if (fieldType == AttributeRiskScoreMode.OperandMetric)
                {
                    ...
                    int mectricto = int.MaxValue;
                    string mectric = string.Empty;
                    if (oper.SelectedItem.Text.Trim().Contains("<>"))
                    {
                        mectricfrom = int.Parse(mectric_from.Text);
                        mectricto = int.Parse(mectric_to.Text);
                        mectric = string.Format("{0};{1}", mectricfrom, mectricto);
                    ...
                    {
                        if (oper.SelectedItem.Text.Trim().StartsWith("<"))
                            mectricto = int.Parse(mectric_from.Text);
                        else mectricfrom = int.Parse(mectric_from.Text);
                        mectric = mectric_from.Text;
                    }
                    aAttr.From = mectricfrom.ToString();
                    aAttr.To = mectricto.ToString();
                    aAttr.Metrics = mectric;
    ...
    private void DisplayControl(GridDataItem item, AttributeRiskScoreMode mode)
    {
        ...
        TextBox mectric_from = item.FindControl(CT_METRIC_FROM) as TextBox;
        TextBox mectric_to = item.FindControl(CT_METRIC_TO) as TextBox;
        Label mectric_from_lable = item.FindControl(CT_METRIC_FROM_LABEL) as Label;
        Label mectric_to_lable = item.FindControl(CT_METRIC_TO_LABEL) as Label;
        HiddenField mectric_MinValue = item.FindControl("hddMinValueFromTo") as HiddenField;
        HiddenField mectric_MaxValue = item.FindControl("hddMaxValueFromTo") as HiddenField;
        ...
        switch (mode)
        {
            case AttributeRiskScoreMode.OperandMetric:
                ...
                ShowHideOperandMetric(Operand, item);
                break;
            case AttributeRiskScoreMode.Operand:
            case AttributeRiskScoreMode.SingleSelect:
            case AttributeRiskScoreMode.OperandIfEmpty:
                ...
                metric_textListPanel.Visible = false;
                metric.Visible = true;
                mectric_from.Visible = mectric_to.Visible = mectric_from_lable.Visible = mectric_to_lable.Visible = false;
                break;
            case AttributeRiskScoreMode.FromTo:
                ...
                string minValue = attribute.SelectedItem.Attributes["MinValue"].ToString();
                string maxValue = attribute.SelectedItem.Attributes["MaxValue"].ToString();

                mectric_MinValue.Value = minValue;
                mectric_MaxValue.Value = maxValue;
                break;
    ...
    private void LoadOperand(string value, GridDataItem item)
    {
        ...
        {
            if (fieldType != AttributeRiskScoreMode.SingleSelect)
                LoadMetric(value, "-1", "", item, fieldType);
            else
                LoadMetric(value, "-1", "SingleSelect", item, fieldType);
        }
    ...
    private DataTable GetAttributes()
    {
        ...
        tb = WebServices.RiskServices.GetReports("spa_RM_MCF_MRS_Get_AttributeList", prams);
    ...
    public void ShowHideOperandMetric(RadComboBox combo, GridDataItem item)
    {
        RadComboBox attribute = item.FindControl(CT_ATTRIBUTE) as RadComboBox;
        TextBox mectric_from = item.FindControl(CT_METRIC_FROM) as TextBox;
        TextBox mectric_to = item.FindControl(CT_METRIC_TO) as TextBox;
        ...
        string minValue = attribute.SelectedItem.Attributes["MinValue"].ToString();
        string maxValue = attribute.SelectedItem.Attributes["MaxValue"].ToString();
        HiddenField mectric_MinValue = item.FindControl("hddMinValueMetric") as HiddenField;
        HiddenField mectric_MaxValue = item.FindControl("hddMaxValueMetric") as HiddenField;

        if (!string.IsNullOrEmpty(combo.SelectedValue) && combo.SelectedItem.Text.Trim().Contains("<>"))
        {
            mectric_from.Visible = mectric_to.Visible = mectric_from_lable.Visible = mectric_to_lable.Visible = true;
            mectric_from.Width = mectric_to.Width = 85;
        }
        else
        {
            mectric_from.Visible = true;
            mectric_to.Visible = mectric_from_lable.Visible = mectric_to_lable.Visible = false;
            mectric_from.Width = 130;
        }

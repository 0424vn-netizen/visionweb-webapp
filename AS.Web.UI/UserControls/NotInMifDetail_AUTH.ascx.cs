...
public partial class UserControls_NotInMifDetail_AUTH : GlobalUserControl
{
    enum DataBindAction
    {
        BindReportGrid,
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            uxltFileType.Text = VeraCodeSolution.DoVeraCode(GetLocalResourceObject("FileTypeAuth").ToString());
            uxltReportDate.Text = ReportDate.ToString("MM/dd/yyyy");

            OnDataBindControls(DataBindAction.BindReportGrid, uxNotInMifDetailGridAuth);
    ...
    private void DoSwitchView()
    {
        ...
        {
            uxNotInMifDetailGridAuth.Columns.FindByUniqueName(CARD_NUMBER).Visible = true;
            uxNotInMifDetailGridAuth.Columns.FindByUniqueName(PARTIAL_CARD_NUMBER).Visible = false;

            if (IsShowRoutingAccount)
            {
                uxNotInMifDetailGridAuth.Columns.FindByUniqueName("RoutingAccountNumber").Visible = true;
                uxNotInMifDetailGridAuth.Columns.FindByUniqueName("PartialRoutingACC").Visible = false;
            }
        }
        else
        {
            uxNotInMifDetailGridAuth.Columns.FindByUniqueName(CARD_NUMBER).Visible = false;
            uxNotInMifDetailGridAuth.Columns.FindByUniqueName(PARTIAL_CARD_NUMBER).Visible = true;

            if (IsShowRoutingAccount)
            {
                uxNotInMifDetailGridAuth.Columns.FindByUniqueName("RoutingAccountNumber").Visible = false;
                uxNotInMifDetailGridAuth.Columns.FindByUniqueName("PartialRoutingACC").Visible = true;
            }
        }
    }
            ...
            case DataBindAction.BindReportGrid:
                {
                    ...
                    ((ASGrid)sender).DataSourceInvoker = new AS.Controls.Grid.ASFuncInvoker(WebServices.CsReportServices, WebSiteConstants.GET_REPORT_METHOD_NAME, new object[] { "spa_MerchantsNotInMif_Get_AuthorizationDetail", ReportServices.ConvertToFilterParamWSArray(parames) });
                    break;
                }
        }
    }
    ...
    protected void uxNotInMifDetailGrid_ItemDataBound(object sender, GridItemEventArgs e)
    {
        if (e.Item is GridDataItem)
        {
            ...
            if (GeneralFuncsLib.CheckCSViewFullCard((SecurePage)this.Page) && (!dataItem[CARD_NUMBER].Text.Equals(GeneralFuncsLib.NBSP) || !dataItem["RoutingAccountNumber"].Text.Equals(GeneralFuncsLib.NBSP)))
            {
            ...
            //AD
            dataItem["AD"].ToolTip = VeraCodeSolution.DoVeraCode(dataRow["ADDescription"].ToString());
            if (dataRow["AD"].ToString() == "D")
            {
                dataItem["AD"].Text = string.Format(WebSiteConstants.DECLINED_TEXT, dataItem["AD"].Text);
            }
        }
    }
    ...
    protected void uxExporterAuth_NeedExportConfig(object sender, AS.Controls.Exporter.ExportConfig exportConfig)
    {
        _isExporting = true;
        ...
        uxNotInMifDetailGridAuth.Columns.FindByUniqueName("CardNumber").Visible = false;
        uxNotInMifDetailGridAuth.Columns.FindByUniqueName("PartialCardNumber").Visible = true;

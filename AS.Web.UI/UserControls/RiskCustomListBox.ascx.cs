...
public class CustomListBoxItem : GlobalUserControl
{
    #region Constants
    ...
    public CustomListBoxItem(RepeaterItem rItem)
    {
        _repeaterItem = rItem;
    }
...
public class CustomListBoxItemEventArgs : EventArgs
{
    public CustomListBoxItem Item { get; set; }
}

public partial class CustomListBox : GlobalUserControl
{
    #region Properties
    ...
    private List<CustomListBoxItem> _selectedItems = null;
    public List<CustomListBoxItem> SelectedItems
    {
        get
        {
            if (_selectedItems == null)
            {
                _selectedItems = new List<CustomListBoxItem>();
                foreach (RepeaterItem item in uxRepeater.Items)
                {
                    var lItem = new CustomListBoxItem(item);
                    if (lItem.Selected)
    ...
    private List<CustomListBoxItem> _items = null;
    public List<CustomListBoxItem> Items
    {
        get
        {
            if (_items == null)
            {
                ...
                foreach (RepeaterItem item in uxRepeater.Items)
                {
                    _items.Add(new CustomListBoxItem(item));
                }
            }
            return _items;
        }
    ...
    public delegate void ItemDataBoundEvent(CustomListBox sender, CustomListBoxItemEventArgs e);
    ...
    protected void uxRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        ...
        if (ItemDataBound != null)
            ItemDataBound(this, new CustomListBoxItemEventArgs() { Item = new CustomListBoxItem(e.Item) });
    }

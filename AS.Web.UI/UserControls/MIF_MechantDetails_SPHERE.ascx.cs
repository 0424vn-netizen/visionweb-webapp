...
public partial class UserControls_MIF_MechantDetails_SPHERE : ExportMultiSections
{
    #region Enums
    ...
    private const string FILE_NAME_BUSINESS_INFO = "~/App_Data/tpl_BusinessInformation_SPHERE.htm";
    private const string FILE_NAME_HIERARCHY_INFO = "~/App_Data/tpl_HierarchyInformation_SPHERE.htm";
    private const string FILE_NAME_ACCOUNT_INFO = "~/App_Data/tpl_AccountInformation_FULTON.htm";
    //TK 41447
    private const string FILE_NAME_BANK_INFO = "~/App_Data/tpl_BankInformation_MON.htm";
    private const string FILE_NAME_BANK_PART_INFO = "~/App_Data/tpl_BankInformation_MON_Part.htm";
    private const string FILE_NAME_MERCHANT_INFO = "~/App_Data/tpl_MerchantInformation_SPHERE.htm";
    private const string FILE_NAME_MERCHANT_INFO_REP = "~/App_Data/tpl_MerchantInformation_ORI_Rep.htm";
    // SPA
    private const string SPA_GET_MERCHANT_PROFILE = "spa_cs_GetMerchantProfile_TSYS_Detail_SPHERE";
    //TK 41447
            ...
            case DataBindAction.BindMerchantDetails:
                ...
                if (dtMerch.Rows.Count > 0)
                {
                    ...
                    foreach (RepeaterItem item in rptMerchantInfo.Items)
                    {
                        ...
                        MerchantProfileHelper.BuildMIFDetailHierarchyLink(dtMerch, item, "uxLinkSPHEREFUND", "EntityNumber2", HierarchyMode.SPHERE_FUNDINGMETHOD); 
                        MerchantProfileHelper.BuildMIFDetailHierarchyLink(dtMerch, item, "uxLinkAssociation", "EntityNumber3", HierarchyMode.SPHERE_ASSOCIATION);
                        MerchantProfileHelper.BuildMIFDetailHierarchyLink(dtMerch, item, "uxLinkBank", "EntityNumber1", HierarchyMode.SPHERE_BANK); 
                        MerchantProfileHelper.BuildMIFDetailHierarchyLink(dtMerch, item, "uxLinkReferralPartner", "EntityNumber4", HierarchyMode.SPHERE_REF);
                        MerchantProfileHelper.BuildMIFDetailHierarchyLink(dtMerch, item, "uxLinkGroup", "EntityNumber5", HierarchyMode.SPHERE_GRP);
                        MerchantProfileHelper.BuildMIFDetailHierarchyLink(dtMerch, item, "uxLinkReferralSource", "EntityNumber6", HierarchyMode.SPHERE_REFS);                      
                    }
                }
                else
                {
    ...
    protected override Dictionary<int, Func<string>> GetListFunctions()
    {
        ...
        else
        {
            ...
            exportFucntions.Add(3, ButtonExcelAccountInformation_Click);
    ...
    public void DoMultiExportExcel()
    {
        ...
        else
        {
            ...
            exportNames.Add(3, GetLocalResourceObject("MIF_MerchantDetails_FULTONCS_Text_AccountInformation").ToString());
            exportNames.Add(4, GetLocalResourceObject("MIF_MerchantDetails_FULTONCS_Text_BankInformation").ToString());
        }
    ...
    protected string ButtonExcelBusinessInformation_Click()
    {
        ...
        strExcelTemplate.Replace("[tpl_BusinessInformation_htm_OpenDate]", Resources.Template.tpl_BusinessInformation_ORION_htm_ApprovalDate);
        ...
        strExcelTemplate.Replace("BI_CORPORATENAME", "&nbsp;" + (dataSource.Rows[0]["CorporateName"] != DBNull.Value && !string.IsNullOrEmpty(dataSource.Rows[0]["CorporateName"].ToString()) && !dataSource.Rows[0]["CorporateName"].ToString().Equals("n/a", StringComparison.OrdinalIgnoreCase) ? dataSource.Rows[0]["CorporateName"].ToString() : WebSiteConstants.HTML_EM_DASH));
    ...
    protected string ButtonExcelHierarchyInformation_Click()
    {
        ...
        strExcelTemplate.Replace("[tpl_HierarchyInformation_htm_MerchantHierarchy]", Resources.Template.tpl_HierarchyInformation_Fulton_htm_MerchantHierarchy);
        strExcelTemplate.Replace("[tpl_HierarchyInformation_htm_Bank]", Resources.Template.tpl_HierarchyInformation_htm_Bank);
        strExcelTemplate.Replace("[tpl_HierarchyInformation_htm_FundingMethod]", Resources.Template.tpl_HierarchyInformation_htm_FundingMethod);
        strExcelTemplate.Replace("[tpl_HierarchyInformation_htm_Association]", Resources.Template.tpl_HierarchyInformation_Fulton_htm_Association);
        strExcelTemplate.Replace("[tpl_HierarchyInformation_htm_ReferralPartner]", Resources.Template.tpl_HierarchyInformation_htm_ReferralPartner);
        strExcelTemplate.Replace("[tpl_HierarchyInformation_htm_Chain]", Resources.Template.tpl_HierarchyInformation_Fulton_htm_Chain);
        strExcelTemplate.Replace("[tpl_HierarchyInformation_htm_Group]", Resources.Template.tpl_HierarchyInformation_htm_Group);
        strExcelTemplate.Replace("[tpl_HierarchyInformation_htm_ReferralSource]", Resources.Template.tpl_HierarchyInformation_htm_ReferralSource);
        strExcelTemplate.Replace("[tpl_HierarchyInformation_htm_ReferralPartnerUserID]", Resources.Template.tpl_HierarchyInformation_htm_ReferralPartnerUserID);
        strExcelTemplate.Replace("[tpl_HierarchyInformation_htm_ReferralSourceUserID]", Resources.Template.tpl_HierarchyInformation_htm_ReferralSourceUserID);
        ...
        // Merchant Hierarchy
        strExcelTemplate.Replace("HI_BANK", "&nbsp;" + dataSource.Rows[0]["BankName"].ToString());
        strExcelTemplate.Replace("HI_FUNDINGMETHOD", "&nbsp;" + dataSource.Rows[0]["EntityNumber2"].ToString());
        strExcelTemplate.Replace("HI_ASSOCIATION", "&nbsp;" + dataSource.Rows[0]["AssoName"].ToString()); 
        strExcelTemplate.Replace("HI_REFERRALPARTNER", "&nbsp;" + dataSource.Rows[0]["EntityNumber4"].ToString()); 
        strExcelTemplate.Replace("HI_CHAIN", "&nbsp;" + dataSource.Rows[0]["Chain"].ToString());
        strExcelTemplate.Replace("HI_GROUP", "&nbsp;" + dataSource.Rows[0]["GroupName"].ToString());
        strExcelTemplate.Replace("HI_REFERRALSOURCE", "&nbsp;" + dataSource.Rows[0]["EntityNumber6"].ToString());
        strExcelTemplate.Replace("HI_ReferralPartnerUserID", "&nbsp;" + 
            (string.IsNullOrEmpty(dataSource.Rows[0]["RefPartnerUserID"].ToString())? "-": dataSource.Rows[0]["RefPartnerUserID"].ToString()));
        strExcelTemplate.Replace("HI_ReferralSourceUserID", "&nbsp;" + 
            (string.IsNullOrEmpty(dataSource.Rows[0]["RefSourceUserID"].ToString())?"-": dataSource.Rows[0]["RefSourceUserID"].ToString()));
    ...
    protected string ButtonExcelAccountInformation_Click()
    {
        ...
        strExcelTemplate.Replace("[tpl_AccountInformation_FULTON_htm_Status]", Resources.Template.tpl_AccountInformation_FULTON_htm_Status);
        strExcelTemplate.Replace("[tpl_AccountInformation_FULTON_htm_EffectiveDate]", Resources.Template.tpl_AccountInformation_FULTON_htm_EffectiveDate);
        strExcelTemplate.Replace("AI_ACCOUNT_INFO", "&nbsp;" + GetLocalResourceObject("MIF_MerchantDetails_FULTONCS_Text_DiscoverMap").ToString());
        strExcelTemplate.Replace("AI_STATUS", "&nbsp;" + FormatStatusAccountInfo(_datasource.Rows[0]["DiscoverRetained"]));
        strExcelTemplate.Replace("AI_EFFECTIVE_DATE", "&nbsp;"
            + FormatEffectiveDate(_datasource.Rows[0]["DiscoverRetainedEffectiveDate"],
                                  _datasource.Rows[0]["DiscoverRetained"]));
    ...
    protected string ButtonExcelBankInformation_Click()
    {
        ...
        // Bank part
        StringBuilder strBankPart = new StringBuilder(File.ReadAllText(Server.MapPath(FILE_NAME_BANK_PART_INFO)));
        //loop
        StringBuilder strBank = new StringBuilder();

        DataTable bankInfor = (DataTable)(BankInformationTable);

        if (bankInfor == null || bankInfor.Rows.Count == 0)
        {
            string str = strBankPart.ToString();
            str = str.Replace("[BA_Routing]", "&nbsp;" + WebSiteConstants.HTML_EM_DASH);
            str = str.Replace("[BA_DDA]", "&nbsp;" + WebSiteConstants.HTML_EM_DASH);

            strBank.AppendLine(str.ToString());
        }
        else
        {
            foreach (DataRow row in bankInfor.Rows)
            {
                string str = strBankPart.ToString();
                str = str.Replace("[BA_Routing]", "&nbsp;" + FormatDataToMDash(row["PartialRoutingNumber"].ToString()));
                str = str.Replace("[BA_DDA]", "&nbsp;" + FormatDataToMDash(row["PartialDDANumber"].ToString()));

                strBank.AppendLine(str.ToString());
            }
        }

        strExcelTemplate.Replace("[BANK_PART]", strBank.ToString());

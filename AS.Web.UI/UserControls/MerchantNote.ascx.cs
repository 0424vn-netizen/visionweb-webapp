...
public partial class UserControls_MerchantNote : GlobalUserControl
{
    #region contanst
    private const string MERCHANT_PROFILE_PAGE = "MerchantProfile.aspx";
    private const string RISK_REPORT_PAGE = "RiskReport.aspx";
    private const string SORT_DEFAULT_VALUE = "CreationDate DESC";
    ...
    private const string SPA_GET_MERCHANT_LIST = "spa_MerchantNotes_GetList";
    #endregion
    #region enum
    ...
    public MerchantNoteFilterOptionExtend MerchantNoteFilterOption
    {
        get
        {
            if (ViewState["MerchantNoteFilterOption"] == null)
            {
                return new MerchantNoteFilterOptionExtend()
                {
            ...
            {
                return ViewState["MerchantNoteFilterOption"] as MerchantNoteFilterOptionExtend;
            }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            LoggerManager.Error(string.Format("UserControls_MerchantNote - ViewStateException - Request={0}; SessionID={1}", Context.Request.Url, Session.SessionID));
            throw;
        }
    }
            ...
            case DataBindAction.BindGridMerchantNote:
                {
                    ...
                    parameters.Add(new FilterParameter("@NotesSourceList", MerchantNoteFilterOption.NotesSourceList, DbType.AnsiString));
                    parameters.Add(new FilterParameter("@RoleList", MerchantNoteFilterOption.RoleList, DbType.AnsiString));
                    parameters.Add(new FilterParameter("@UserList", MerchantNoteFilterOption.UserList, DbType.AnsiString));
                    ...
                    parameters.Add(new FilterParameter("@stOrder", SessionManager.MerchantNoteSort, DbType.AnsiString));
                    parameters.AddLanguageID();
                    ((ASGrid)sender).DataSourceInvoker = new AS.Controls.Grid.ASFuncInvoker(WebServices.CsReportServices, WebSiteConstants.GET_REPORT_METHOD_NAME,
                        new object[] { SPA_GET_MERCHANT_LIST, ReportServices.ConvertToFilterParamWSArray(parameters) });
                    break;
                }
        }
    }
    ...
    protected void uxExportTop_NeedExportConfig(object sender, AS.Controls.Exporter.ExportConfig exportConfig)
    {
        ...
        //uxMerchantNoteGrid.Columns.FindByUniqueName("NoteTitle").Visible = true;
        ...
        uxMerchantNoteGrid.Columns.FindByUniqueName("UserNameFull").Visible = true;
        uxMerchantNoteGrid.Columns.FindByUniqueName("CreationDate").Visible = true;
    }
    ...
    protected void uxMerchantNoteGrid_ItemDataBound(object sender, GridItemEventArgs e)
    {
        ...
        if (dataItem.IsNotNullData())
        {
            ...
            colCaseIdColumn.InnerHtml = caseIdHyperlink;
            colcomment.InnerHtml = rowView["Comment"].ToString().Replace("&nbsp;", " ");
            colAddedByColumn.InnerText = string.Format("{0} - {1}", rowView["UserNameFull"].ToString(), rowView["HierarchyName"].ToString());

            if (rowView["CaseID"].IsNullOrEmpty())
            {
                HtmlGenericControl divCase = cardViewColumnCtr.FindControl("ucCase") as HtmlGenericControl;
                divCase.Attributes.Add("class", "vis-hidden col-md-3");
    ...
    {
        MerchantNoteFilterOptionExtend m = new MerchantNoteFilterOptionExtend();
...
[Serializable]
public class MerchantNoteFilterOptionExtend
{

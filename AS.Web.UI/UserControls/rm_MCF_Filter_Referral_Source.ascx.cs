...
public partial class UserControls_rm_MCF_Filter_Referral_Source : GlobalUserControl, IRiskParamFilter
{
    #region Enums
    enum DataBindAction
    {
    ...
    #endregion
    static string PAGE_PATH = "~/UserControls/rm_MCF_Filter_Referral_Source.ascx";
    ...
    {
        get { return uxReferralSource.GetSelectedItemsAsString(","); }
    }
    ...
    {
        MESSAGE_MODE = "<b>" + GetLocalResourceObject("Risk_Filter_Referral_Source_ascx_cs_MessageCode").ToString() + "</b><br />";
        _includedIn = GetLocalResourceObject("Risk_Filter_Referral_Source_ascx_cs_IncludedIn").ToString();
        _excludedFrom = GetLocalResourceObject("Risk_Filter_Referral_Source_ascx_cs_ExcludedFrom").ToString();
        _defaultValueNA = GetLocalResourceObject("Risk_Filter_Referral_Source_ascx_cs_NA").ToString();
        if (!IsPostBack)
        {
            //BindData();
    ...
    private void BindData()
    {
        ...
        uxReferralSource.DataSourceOrigination = listOri;
        uxReferralSource.DataSourceDestination = listDes;
    }
    ...
    private DataTable GetFilterList(WebSiteEnums.AssignmentFilterModes whichMode)
    {
        ...
        return WebServices.RiskServices.GetReports("spa_RM_MCF_GetFilterReferralSource", paramsIn);
    }
    ...
    public static Dictionary<string, string> GetSelectedValuesAsString(WebSiteEnums.ParamFilterMode mode, string primaryID, string paramID, string FilterID)
    {
        ...
        DataSet ds = WebServices.RiskServices.GetReportsAsDataSet("spa_RM_MCF_GetFilterReferralSource", paramsIn);
    ...
    private static Dictionary<string, string> BuildeSelectedValues(DataTable dt, bool isInclude)
    {
        ...
        MESSAGE_MODE = "<b>" + HttpContext.GetLocalResourceObject(PAGE_PATH, "Risk_Filter_Referral_Source_ascx_cs_MessageCode").ToString() + "</b><br />";
        _includedIn = HttpContext.GetLocalResourceObject(PAGE_PATH, "Risk_Filter_Referral_Source_ascx_cs_IncludedIn").ToString();
        _excludedFrom = HttpContext.GetLocalResourceObject(PAGE_PATH, "Risk_Filter_Referral_Source_ascx_cs_ExcludedFrom").ToString();
        _defaultValueNA = HttpContext.GetLocalResourceObject(PAGE_PATH, "Risk_Filter_Referral_Source_ascx_cs_NA").ToString();
    ...
    public int Save()
    {
        ...
        WebServices.RiskServices.ExecuteNonQueryCommand("spa_RM_MCF_SaveReferralSourceAssignment", paramsIn, out paramsOut);

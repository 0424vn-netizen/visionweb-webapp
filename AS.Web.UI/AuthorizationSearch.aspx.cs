...
[PagePermission("TCAuth,TCFlatAuth")]
public partial class AuthorizationSearch : ReportPage
{
    #region Enums
    enum DataBindAction
    {
    ...
    {
        get { return (string)ViewState["_TerminalID"]; }
        set { ViewState["_TerminalID"] = value; }
    }
    ...
    private string _ProcessingCode { get; set; }

    private decimal? _AuthAmount
    {
        get { return (decimal?)ViewState["_AuthAmount"]; }
        set { ViewState["_AuthAmount"] = value; }
    }
    ...
    const string AUTH_DETAIL = "<a href=\"#\" style=\"cursor:pointer\" onclick=\"return ShowPopupModal('{0}','max');\">{1}</a>";
    const string CAPTURE_DETAIL = "<a href=\"#\" style=\"cursor:pointer\" onclick=\"return ShowPopupModal('{0}','max');\"><img src='res/images/view.png' border=\"0\"/></a>";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                uxRadAccountNumber.Text = VeraCodeSolution.DoVeraCode(GetLocalResourceObject("Text_ANLast4").ToString());
            }
            SetDefaultDate();
            BindProcessingCode();
            ProcessParams();
        }
        //Value greater than Today not allowed
    ...
    protected override void DoItemDataBound(ASGrid sender, Telerik.Web.UI.GridItemEventArgs e)
    {
        ...
        if (e.Item is GridDataItem)
        {
            ...
            {
                dataItem["MerchantNumber"].Text = string.Empty;
                dataItem["MerchantName"].Text = string.Empty;
            }
            else
            {
            ...
            string authUrl = "AuthorizationSearchDetail.aspx?{0}";
            ...
            if (IsUserWithPermission(WebSiteConstants.SEC_PERMISSION_CC))
            {
                ...
                string accountNumber = Cryptophy.EncryptText(dataRow["AccountNumber"].ToString(), key);
                queryString = this.BuildSecureQueryString("AccountNumber=" + HttpUtility.UrlEncode(accountNumber) + "&BeginDate=" + _BeginDate + "&EndDate=" + _EndDate + "&CardDescription=" + dataRow["CardDescription"].ToString() + "&RecordID=" + dataRow["RecordID"].ToString() + AuthIntruderQuery(uxReportGrid, "AccountNumber"));
                authUrl = string.Format(authUrl, queryString);
                dataItem["AccountNumber"].Text = VeraCodeSolution.DoVeraCode(string.Format(AUTH_DETAIL, authUrl, dataRow["AccountNumber"].ToString()));
            }
            else
            {
                queryString = this.BuildSecureQueryString("AccountNumber=" + dataRow["PartialCardNumber"].ToString() + "&BeginDate=" + _BeginDate + "&EndDate=" + _EndDate + "&CardDescription=" + dataRow["CardDescription"].ToString() + "&RecordID=" + dataRow["RecordID"].ToString() + AuthIntruderQuery(uxReportGrid, "PartialCardNumber"));
                authUrl = string.Format(authUrl, queryString);
                dataItem["PartialCardNumber"].Text = VeraCodeSolution.DoVeraCode(string.Format(AUTH_DETAIL, authUrl, dataRow["PartialCardNumber"].ToString()));
            }

            captureUrl = string.Format(captureUrl, queryString);
            ...
            case DataBindAction.BindReportGrid:
                {
                    ...
                    _params.Add(new FilterParameter("@TerminalNumber", _TerminalID, DbType.AnsiString));
                    _params.Add(new FilterParameter("@AuthAmount", _AuthAmount, DbType.Decimal));
                    _params.Add(new FilterParameter("@ProcessingCode", _ProcessingCode, DbType.AnsiString));
                    _params.AddLanguageID();
                    string spaName = "spa_cs_GetAuthorizationSearch";
    ...
    private void ProcessParams()
    {
        ...
        if (IsUserWithPermission(WebSiteConstants.SEC_PERMISSION_CC))
            _FullNumber = VeraCodeSolution.ValidateResponseData(uxRadAccountNumber.Checked ? uxFilterValue.Text.Trim() != string.Empty ? uxFilterValue.Text.Trim() : null : null);
        else
            _Last4Number = VeraCodeSolution.ValidateResponseData(uxRadAccountNumber.Checked ? uxFilterValue.Text.Trim() != string.Empty ? uxFilterValue.Text.Trim() : null : null);
        _AuthID = VeraCodeSolution.ValidateResponseData(uxAuthID.Text.Trim() != string.Empty ? uxAuthID.Text.Trim() : null);
        _TerminalID = VeraCodeSolution.ValidateResponseData(uxTerminalID.Text.Trim() != string.Empty ? uxTerminalID.Text.Trim() : null);

        if (uxAuthAmount.Text != string.Empty)
        {
            _AuthAmount = Convert.ToDecimal(uxAuthAmount.Value);
        }
        else
        {
            _AuthAmount = null;
        ...
        _ProcessingCode = uxProcCodeDes.SelectedValue != string.Empty ? uxProcCodeDes.SelectedValue : null;
    }
    ...
    private void BindProcessingCode()
    {
        FilterParameterCollection _parames = new FilterParameterCollection();
        DataTable dt = WebServices.CsReportServices.GetReports("spa_cs_GetProcessingCode", _parames);

        uxProcCodeDes.DataSource = dt;
        uxProcCodeDes.DataBind();
        uxProcCodeDes.Items.Insert(0, new RadComboBoxItem(GetLocalResourceObject("Text_SelectOne").ToString(), null));
    }

    private string _AuthIntruderQuery = string.Empty;
    private string AuthIntruderQuery(ASGrid grid, string column)
    {
        if (this._AuthIntruderQuery == string.Empty)
            this._AuthIntruderQuery = GeneralFuncsLib.BuildIntruderInfoForQueryStr(grid.ID, new string[] { column });

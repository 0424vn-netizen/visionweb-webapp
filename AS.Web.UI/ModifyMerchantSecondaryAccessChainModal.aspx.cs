...
public partial class ModifyMerchantSecondaryAccessChainModal : NonReportPage
{
    enum PostBackAction
    {
        UpdateChain
    ...
    string _AccessChainNumber = string.Empty;
    private void ProcessQueryString()
    {
        if (IsSecureQueryString)
        {
            _MerchantNumber = SecureQueryString["MerchNum"];
            _AccessChainNumber = SecureQueryString["AccessChain"];
        }
    }
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                ProcessQueryString();
                if (_AccessChainNumber.Trim().IsNullOrEmpty())
                {
                ...
                {
                    uxLtrDetails.Text = VeraCodeSolution.DoVeraCode(
                        string.Format(GetLocalResourceObject("ModifyMerchantChainModal_aspx_cs_MerchantIsPartOfChain").ToString(), _MerchantNumber, _AccessChainNumber)
                        );
    ...
    protected void UpdateChain()
    {
        if (IsSecureQueryString)
        {
            string chainSrc = SecureQueryString["AccessChain"].ToString();
            string merchantNumber = SecureQueryString["MerchNum"];
            string spaName = "spa_cs_UpdateSecondaryAccessChain";
    ...
    protected void uxChain_OnItemsRequested(object sender, RadComboBoxItemsRequestedEventArgs e)
    {
        if (e.Text.Length > 2)
        {
            string spaName = "spa_cs_GetSecondaryAccessChain";
            FilterParameterCollection parameters = new FilterParameterCollection();
            parameters.AddLoggedInUserReportingParams(true);

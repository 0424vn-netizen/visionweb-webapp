...
namespace AS.VW.Api.WCF.ServiceInterface
{
    ...
    public interface IMerchantInformationService
    {
        #region General
        [OperationContract]
        [ApiPermission(PermissionCode=WebApiSettings.PERMISSION_MIF)]
        ReportResponse<MerchantSummary> GetMerchantList(HierarchyPagingFilter req);

        [OperationContract]
        [ApiPermission(PermissionCode = WebApiSettings.PERMISSION_MIF)]
        ReportResponse<MerchantComment> GetMerchantComments(MerchantInformationPagingFilter req);
        #endregion
